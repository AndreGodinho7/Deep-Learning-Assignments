...
def main():
    ...
    hidden_size_v = [100,200]
    drop_prob = [0.3,0,5]
    activ_func = ['tanh','relu']
    optimizer_used = ['adam','sgd']

    final_acc = []

    for i in range(len(learning_rates)):
        opt.learning_rate = learning_rates[i]
        for j in range(len(hidden_size_v)):
            #for k in range(len(drop_prob)):
            #    for l in range(len(activ_func)):
            #        for j in range(len(optimizer_used)):
            opt.l2_decay = l2_decays[j]
            ...
            final_acc_iter = evaluate(model, test_X, test_y)
            print('Final Test acc: %.4f' % (final_acc_iter))

            try:
                if final_acc_iter > final_acc:
                    final_acc = final_acc_iter
                    l2_decay_final = opt.l2_decay
                    lr_final = opt.learning_rate
                    print("Found a new best accuracy.")
            except:
                final_acc = final_acc_iter

            # plot
            plot(epochs, train_mean_losses, ylabel='Loss', name='training-loss ,' + 'lr = '+str(opt.learning_rate)+', l2 = '+str(opt.l2_decay))
            plot(epochs, valid_accs, ylabel='Accuracy', name='validation-accuracy ,'+str(opt.learning_rate)+', l2 = '+str(opt.l2_decay))
            
    print("")
    print('Best accuracy: %.4f' % (final_acc))
    print('L2 decay was: {}'.format(l2_decay_final))
    print('Learning rate was: {}'.format(lr_final))
