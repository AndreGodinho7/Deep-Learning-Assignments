            ...
            for t in range(L):
                if yseq_hat[t] != yseq[t]:
                    ...
                    if t == 0:
                        ...
                        self.W_bigrams[yseq_hat[t],yseq_hat[t-1]] -= phi_transition[t-1][yseq_hat[t],yseq_hat[t-1]]
                        self.W_bigrams[yseq[t],yseq[t-1]] += phi_transition[t-1][yseq[t], yseq[t-1]]
        ...
        if yseq_hat != yseq:
            for t in range(L):
                if yseq_hat[t] != yseq[t]:
                    ...
                    u_gradient = np.dot(emission_posteriors[:, yseq_hat[t]].T, xseq)
                    self.W_unigrams[yseq_hat[t]] -= learning_rate * u_gradient
                    
                    if t == 0:
                        self.W_start[yseq_hat[t]] -= learning_rate * emission_posteriors[0, yseq_hat[t]]
                        self.W_start[yseq[t]] += learning_rate * emission_posteriors[0, yseq[t]]

                    else:
                        self.W_bigrams[yseq[t],yseq[t-1]] += learning_rate * phi_transition[t-1][yseq[t], yseq[t-1]]
                        t_gradient = np.dot(phi_transition[t-1], transition_posteriors[t-1]) 
                        self.W_bigrams[yseq_hat[t],yseq_hat[t-1]] -= learning_rate * t_gradient.sum()
                        if t == L-1: 
                            self.W_stop[yseq[t]] += learning_rate * emission_posteriors[-1, yseq[t]]
                            self.W_stop[yseq_hat[t]] -= learning_rate * emission_posteriors[-1, yseq_hat[t]]
            return 1
        return 0
