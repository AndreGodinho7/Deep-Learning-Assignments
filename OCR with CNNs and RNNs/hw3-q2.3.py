...
def evaluate(model, X, y, decision=0):
    ...
    for xseq, yseq in zip(X, y):
        ...
        if decision == 0: # # mistakes = # different characters between yseq and yseq_hat
...
def main():
    ...
    plt.savefig('Validation accuracy %s cnn layer.pdf' % model)

    plt.figure()
    plt.plot(range(1, opt.epochs + 1), train_losses, 'bo-')
    plt.title('Train loss')
    plt.savefig('Train loss %s cnn layer.pdf' % model)
    
    print('Evaluating...')
    test_acc = evaluate(clf, X_test, y_test)
    with open("Test accuracy BILSTM cnn layer.txt","w") as f:
        print('Test accuracy (cnn layer): %f' % test_acc, file = f)
