...
def main():
        ...
        conv1_weights = model.layer1[0].weight.data.numpy() # cnn first layer weights (tensor values)
        plot_kernels(conv1_weights, 'first layer grey', greyscale = 1)
        
        conv2_weights = model.layer2[0].weight.data.numpy() # cnn second layer weights (tensor values)
